...
DEFAULT_LINE_COLOR = QColor(0, 255, 0, 128) #第四个参数为透明度
...
class Shape(object):
    ...
    #执行点的旋转变换（通过旋转矩阵展开来计算），计算某一点旋转后的坐标
    ...
    def paint(self, painter):
        if self.points:
            ...
            line_path = QPainterPath() #线路径
            vrtx_path = QPainterPath() #顶点路径

            line_path.moveTo(self.points[0]) #线路径先记录标注框的第一个顶点左边
            ...
            painter.drawPath(line_path) #绘制标注框各边
            painter.drawPath(vrtx_path) #绘制标注框各顶点
            painter.fillPath(vrtx_path, self.vertex_fill_color) #填充各顶点
            if self.fill:
                color = self.select_fill_color if self.selected else self.fill_color
                painter.fillPath(line_path, color) #填充标注框

            if self.center is not None:
                center_path = QPainterPath() #标注框中心点路径
                d = self.point_size / self.scale
                center_path.addRect(self.center.x() - d / 2, self.center.y() - d / 2, d, d) #绘制矩形
    ...
    def drawVertex(self, path, i):
        ...
        if shape == self.P_SQUARE:
            path.addRect(point.x() - d / 2, point.y() - d / 2, d, d) #绘制矩形框
        elif shape == self.P_ROUND: 
            path.addEllipse(point, d / 2.0, d / 2.0) #绘制圆
        else:
            assert False, "unsupported vertex shape"
    # def drawVertex(self, path, center):
    #     pass

    #取出某一标注框中与当前鼠标位置接近程度在epsilon内的顶点索引
    ...
    #利用QPainterPath形成标注框路线containsPoint
    def makePath(self):
        path = QPainterPath(self.points[0]) #用指定的起点来创建一个空路径
        for p in self.points[1:]:
            path.lineTo(p) #将直线添加到路径中（组成轮廓的线条）
        return path
